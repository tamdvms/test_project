...
import CollaboratorCategoryForm from "../../compoments/collaboratorCategoryProduct/CollaboratorCategoryForm";

const { CATEGORY_KIND_COLLABORATOR } = categoryKinds;

class CollaboratorCategoryListPage extends ListBasePage {
    initialSearch() {
    ...
    handleRouting(parentId, parentName) {
        ...
        history.push(`${sitePathConfig.collaboratorCategoryProduct.path}?${qs.stringify({...result, parentId, parentName})}`);
    }
    ...
    getList() {
        ...
        const params = { page, size: this.pagination.pageSize, search: this.search, kind: CATEGORY_KIND_COLLABORATOR};
        getDataList({ params });
    }
